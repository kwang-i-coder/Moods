...
router.post('/signin', async(req, res, next) => {
    ...
    try {
        ...
        // 로그인 실패 시 에러 응답
        if (error){
            const err = new Error("로그인에 실패했습니다.");
            err.status = 400;
        ...
        if (userError || !userData) {
            const err = new Error("유저 정보를 찾을 수 없습니다.");
            err.status = 404;
...
router.post('/signin_kakao', async (req, res) => {
    ...
    try {
        ...
        if(userError){
            ...
            userError.message = `userData error: ${userError.message}`;
...
router.post('/send-verification', async(req, res)=>{
    ...
    try {
        ...
        if (!data || !data.user) {
            const err = new Error("인증메일 전송 오류가 발생했습니다.");
            err.status = 500;
...
router.post('/signup', async (req, res) => {
    ...
    try{
        ...
        if(update_error){
            update_error.status = 500;
            update_error.message = `비밀번호 설정 에러: ${update_error.message}`;
...
router.get('/confirm-password-reset', async (req, res) => {
    ...
    if (!token_hash || !type || !next || !email) {
        const err = new Error("필요한 정보가 부족합니다.");
        err.status = 400;
...
router.post('/reset-password', async (req, res) => {
    ...
    try {
        ...
        if(error !== null && error.code === "same_password") {
            const err = new Error("기존 비밀번호와 동일합니다. 다른 비밀번호를 입력해주세요.");
            err.status = 400;
...
router.use((err, req, res, next) => {
    ...
    const message = status === 400 || status === 404 ? (err.message || '잘못된 요청입니다.') : '서버 오류';
