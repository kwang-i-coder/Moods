...
async function getMoodWallpaper(mood_id) {
    if(mood_id.length === 0){
        const { data:url, error } = await supabaseAdmin
            .storage
            .from('wallpaper')
            .createSignedUrl('general/Rectangle 34627910.png', 60*60*24)
    ...
    if(wallpaper_name.length === 0){
        ...
        return { img_path: 'general/Rectangle 34627910.png', url: url.signedUrl, error: null };
    }   
    // 랜덤으로 하나 전송
    ...
    return { img_path: randomWallpaper, url: signedUrl.signedUrl, error: null };
}
